...
void ExportarCsv()
{
    ...
    var lineas = repositorio.ObtenerTodos()
        .OrderBy(p => p.Id)
        .Select(p => $"{p.Id},{p.Nombre},{p.Precio:F2},{p.Cantidad},{p.Categoria},{p.ValorTotal:F2}")
        .ToList();

    lineas.Insert(0, "Id,Nombre,Precio,Cantidad,Categoria,ValorTotal");

    File.WriteAllLines("inventario.csv", lineas);

    Console.WriteLine("\n✓ Inventario exportado a inventario.csv");
}
